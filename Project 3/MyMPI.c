...
int get_size (MPI_Datatype t) {
   ...
   if (t == MPI_LONG_LONG_INT) return sizeof(long long);
...
/*
 *   Function 'my_calloc' is called when a process wants
 *   to allocate some space from the heap. If the memory
 *   allocation fails, the process prints an error message
 *   and then aborts execution of the program.
 */
...
void *my_calloc (
   int id,     			/* IN - Process rank */
   int members,     /* IN - number of members to allocate*/
	 int datum_size)  /* IN - datatype size to allocate */
{
   void *buffer;
   if ((buffer = calloc (members, datum_size)) == NULL) {
      printf ("Error: Malloc failed for process %d\n", id);
      fflush (stdout);
      MPI_Abort (MPI_COMM_WORLD, MALLOC_ERROR);
   }
    ...
    MPI_Comm     comm)   /* IN - Communicator */
{
   ...
   int        i;
   FILE      *infileptr;    /* Input file pointer */
   long long  local_els;    /* Elements on this proc */
   MPI_Status status;       /* Result of receive */
   int        id;           /* Process rank */
   int        p;            /* Number of processes */
   ...
   long long blk_low = (long)id*(*n)/p;
   long long blk_high = (long)(id+1)*(*n)/p - 1;
   local_els = blk_high - blk_low + 1;

   /* Dynamically allocate vector. */

   //*v = my_malloc (id, local_els * datum_size);
   *v = (void *)malloc(local_els * datum_size);
   if (id == (p-1)) {
      for (i = 0; i < p-1; i++) {

		 blk_low = (long)i*(*n)/p;
	     blk_high = (long)(i+1)*(*n)/p - 1;
		 long long send_els = blk_high - blk_low + 1;

         fread (*v, datum_size, send_els,
            infileptr);
         MPI_Send (*v, send_els, dtype, i, DATA_MSG,
            comm);
      }
      fread (*v, datum_size, local_els,
             infileptr);
      fclose (infileptr);
   } else {
      MPI_Recv (*v, local_els, dtype, p-1, DATA_MSG,
         comm, &status);
   }
}
